#include"SolidBuild.h"


// Read Point Coordinates From String
void SolidBuild::SetCoordinate(string point_str, double* coord)
{
	// erase the space
	int index = 0;
	if (!point_str.empty())
...
Loop* SolidBuild::FindCorrectLoop(Loop* testlp, double* coor)
{
...
void SolidBuild::InnerRingCreation(Solid* s, Loop* outerLoop, Loop* assistLp, double* p_out, Value point_arr)
{
	int p = 0;
	double p1[3] = { 0 };
	double p2[3] = { 0 };
...
void SolidBuild::ProcessRing(Solid* s, Loop* outerLoop, double* p_first, Value solid_arr)
{
	Value ring_arr;
...
Solid* SolidBuild::SolidConstruction(string fdir)
{
...
// Test Cases
Solid* SolidBuild::SolidMultiRing()
{
	double points_up_out[][3] = { { 0.0, 60.0, 0.0 },{ 100.0, 60.0, 0.0 },{ 100.0, 160.0, 0.0 },{ 0.0, 160.0, 0.0 } };

	double points_up_in_1[][3] = { {40.0,100.0,0.0},{60.0,100.0,0.0},{60.0,120.0,0.0},{40.0,120.0,0.0} };
	double points_up_in_2[][3] = { {40,125,0,},{60,125,0},{50,140,0} };
	double points_up_in_3[][3] = { {20,110,0,},{35,100,0},{35,120,0} };
	double points_up_in_4[][3] = { {40,95,0,},{60,95,0},{50,80,0} };
	double points_up_in_5[][3] = { {65,100,0,},{65,120,0},{80,110,0} };
	...
	inloop = mef(assistLp, points_up_in_2[2], points_up_in_2[0]);
	kfmrh(outloop, inloop);
	...
	assistLp = myLp;
	mev(assistLp, points_up_out[0], points_up_in_4[0]);
	assistLp = kemr(assistLp, points_up_out[0], points_up_in_4[0]);
	mev(assistLp, points_up_in_4[0], points_up_in_4[1]);
	mev(assistLp, points_up_in_4[1], points_up_in_4[2]);
	inloop = mef(assistLp, points_up_in_4[2], points_up_in_4[0]);
	kfmrh(outloop, inloop);

	assistLp = myLp;
	mev(assistLp, points_up_out[0], points_up_in_5[0]);
	assistLp = kemr(assistLp, points_up_out[0], points_up_in_5[0]);
	mev(assistLp, points_up_in_5[0], points_up_in_5[1]);
	mev(assistLp, points_up_in_5[1], points_up_in_5[2]);
	inloop = mef(assistLp, points_up_in_5[2], points_up_in_5[0]);
	kfmrh(outloop, inloop);
...
Solid* SolidBuild::SolidSingleRing()
{
	double points_up_out[][3] = { { 0.0, 60.0, 0.0 },{ 70.0, 60.0, 0.0 },{ 70.0, 130.0, 0.0 },{ 0.0, 130.0, 0.0 } };
	double points_up_in[][3] = { { 20.0, 80.0, 0.0 },{ 50.0, 80.0, 0.0 },{ 50.0, 110.0, 0.0 },{ 20.0, 110.0, 0.0 } };
...
Solid* SolidBuild::SolidSingleRingTwo()
{
	double points_up_out[][3] = { { 0.0, 60.0, 0.0 },{ 70.0, 60.0, 0.0 },{ 70.0, 130.0, 0.0 },{ 0.0, 130.0, 0.0 } };
	double points_up_in[][3] = { { 10.0, 80.0, 0.0 },{ 60.0, 80.0, 0.0 },{ 35.0, 115.0, 0.0 } };
	...
	mev(assistLp, points_up_in[0], points_up_in[1]);
	mev(assistLp, points_up_in[1], points_up_in[2]);
	Loop* inloop = mef(assistLp, points_up_in[2], points_up_in[0]);
	kfmrh(outloop, inloop);
